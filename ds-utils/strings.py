...
def append_tags_to_frame(x_train: pandas.DataFrame, x_test: pandas.DataFrame, field_name: str, prefix: Optional[str],
                         min_df: float) -> Tuple[pandas.DataFrame, pandas.DataFrame]:
    vectorizer = CountVectorizer(binary=True, tokenizer=_tokenize, encoding="latin1", lowercase=False,
                                 min_df=min_df)
    ...
    x_test_tags = pandas.DataFrame(x_test_count_matrix.A,
                                   columns=["tag_" + tag_name for tag_name in vectorizer.get_feature_names()])
