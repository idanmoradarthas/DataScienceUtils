...
def loan_data() -> pd.DataFrame:
    ...
    return pd.read_csv(
        RESOURCES_PATH.joinpath("loan_final313.csv"),
        encoding="latin1",
        parse_dates=["issue_d"],
    ).drop("id", axis=1)
...
@pytest.mark.parametrize(
    ...
    ids=[
        ...
        "object",
        "category",
        "category_more_than_10_categories",
    ],
)
...
def test_visualize_feature_bool(loan_data):
    ...
    loan_dup["term 36 months"] = (
        loan_data["term"].apply(lambda term: term == " 36 months").astype("bool")
    )
...
def test_visualize_correlations(data_1m, use_existing_ax):
    ...
    corr = data_1m.apply(lambda x: x.factorize()[0]).corr(
...
def test_get_correlated_features():
    ...
    correlations = pd.read_feather(
        RESOURCES_PATH.joinpath("loan_final313_small_corr.feather")
    )
    correlation = get_correlated_features(
        correlations,
        correlations.columns.drop("loan_condition_cat").tolist(),
        "loan_condition_cat",
        0.95,
    )
...
def test_get_correlated_features_empty_result():
    ...
    correlations = pd.read_feather(
        RESOURCES_PATH.joinpath("clothing_classification_train_corr.feather")
    )
    expected_warning = (
        "Correlation threshold 0.95 was too high. An empty frame was returned"
    )
...
def assert_series_called_with(mock_calls, expected_series, percentile):
    """Check if mock was called with a specific Series and percentile."""
    for args, _ in mock_calls:
        series, p = args
        if (
            p == percentile
            and isinstance(series, pd.Series)
            and series.equals(expected_series)
        ):
            return True
    return False
...
def test_extract_statistics_dataframe_per_label_basic_functionality(sample_df, mocker):
    ...
    mock_safe_percentile = mocker.patch(
        "ds_utils.preprocess.safe_percentile", wraps=safe_percentile
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "category",
            TypeError,
            "Feature column 'text_col' must be numeric",
        ),
    ],
    ids=[
...
def test_extract_statistics_dataframe_per_label_exceptions(
    sample_df, feature_name, label_name, exception, message
):
