...
@pytest.fixture
def iris_data() -> tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    """Load and return Iris dataset components for testing."""
    iris_x = pd.read_csv(
        Path(__file__).parents[0].joinpath("resources").joinpath("iris_x_full.csv")
    )
    labels = np.asarray(
        [
            1,
    ...
    cluster_centers = np.asarray(
        [
            [
                6.442105263157895223e00,
                2.978947368421052566e00,
                4.594736842105263008e00,
                1.431578947368421062e00,
            ],
            [
                5.242857142857142883e00,
                3.667857142857142705e00,
                1.499999999999999556e00,
                2.821428571428574728e-01,
            ],
            [
                7.474999999999999645e00,
                3.125000000000000000e00,
                6.299999999999999822e00,
                2.049999999999999822e00,
            ],
            [
                5.620833333333333570e00,
                2.691666666666666430e00,
                4.075000000000000178e00,
                1.262499999999999956e00,
            ],
            [
                6.036842105263158231e00,
                2.705263157894736814e00,
                5.000000000000000000e00,
                1.778947368421052611e00,
            ],
            [
                5.000000000000000000e00,
                2.299999999999999822e00,
                3.274999999999999911e00,
                1.024999999999999911e00,
            ],
            [
                4.704545454545455030e00,
                3.122727272727272574e00,
                1.413636363636363136e00,
                2.000000000000001776e-01,
            ],
            [
                6.568181818181818343e00,
                3.086363636363636420e00,
                5.536363636363637042e00,
                2.163636363636363580e00,
            ],
        ]
    )
...
@pytest.fixture
def distance_wrapper_plot_magnitude_vs_cardinality(mocker) -> Callable:
    """Create a mock distance function that returns pre-loaded distances."""
    with (
        Path(__file__)
        .parents[0]
        .joinpath("resources")
...
def test_plot_cluster_magnitude(
    iris_data, distance_wrapper_plot_magnitude_vs_cardinality
):
...
def test_cluster_magnitude_inconsistent_shapes(mocker, iris_data):
    ...
    with pytest.raises(ValueError, match="X and labels must have the same length."):
        plot_cluster_magnitude(
            iris_x.values[:-1], labels, cluster_centers, mocker.Mock()
        )
...
def test_cluster_magnitude_invalid_distance_function(mocker, iris_data):
    ...
    with pytest.raises(ValueError, match="Invalid distance_function provided."):
        plot_cluster_magnitude(
            iris_x.values, labels, cluster_centers, mocker.Mock(side_effect=TypeError)
        )
...
def test_cluster_magnitude_invalid_cluster_number_vs_labels(mocker, iris_data):
    ...
    with pytest.raises(
        ValueError,
        match="Number of cluster centers must match the number of unique labels.",
    ):
        plot_cluster_magnitude(
            np.array([1]), np.array([1]), cluster_centers, mocker.Mock()
        )
...
def test_magnitude_vs_cardinality_inconsistent_centers(mocker, iris_data):
    ...
    with pytest.raises(
        ...
        plot_magnitude_vs_cardinality(
            iris_x.values, labels, cluster_centers[:-1], mocker.Mock()
        )
...
def test_loss_vs_cluster_number_invalid_algorithm_parameters(mocker, iris_data):
    ...
    with pytest.raises(
        ValueError,
        match="No valid results were obtained. Check your input data and parameters.",
    ):
        plot_loss_vs_cluster_number(
            iris_x.values,
            3,
