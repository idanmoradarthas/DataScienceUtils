"""Explainable AI (XAI) utilities, focusing on tree-based model interpretation."""
import os
import warnings
...
def _recurse(
    ...
    if tree.feature[node] != sklearn_tree.TREE_UNDEFINED:
        ...
        output.write(
            f"{indent}# return class {class_name} with "
            f"probability {prob_array[index]:.4f}{os.linesep}"
        )
        output.write(
            f'{indent}return ("{class_name}", {prob_array[index]:.4f}){os.linesep}'
        )
...
def generate_decision_paths(
    ...
    """Generate decision rules (or 'decision paths') from a decision tree.

    The output is text in valid Python syntax.
    ...
    """
    warnings.warn(
        "This module is deprecated. Use sklearn.tree.export_text instead",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    feature_names = feature_names or [
        f"feature_{i}" for i in range(classifier.n_features_in_)
    ]
    required_features = [
        feature_names[i] if i != sklearn_tree.TREE_UNDEFINED else "undefined!"
    ...
    output = StringIO()
    signature_vars = list(
        dict.fromkeys(f for f in required_features if f != "undefined!")
    )
...
def draw_tree(
    ...
    **kwargs,
    ...
    """
    warnings.warn(
        "This module is deprecated. Use sklearn.tree.plot_tree instead",
        DeprecationWarning,
        stacklevel=2,
    )
...
def draw_dot_data(
    dot_data: str, *, ax: Optional[axes.Axes] = None, **kwargs
) -> axes.Axes:
    """Plot a graph from Graphviz's Dot language string.

    :param dot_data: Graphviz's Dot language string. Use
        `sklearn.tree.export_graphviz` to generate the dot data string.
    :param ax: Axes object to draw the plot onto, otherwise uses the current Axes
    :param kwargs: Additional keyword arguments passed to matplotlib.axes.Axes.imshow()
    :return: Axes object with the plot drawn onto it
...
def plot_features_importance(
    ...
    **kwargs,
    ...
    :param feature_names: Numpy array or list of feature names
        of shape (n_features,).
    :param feature_importance:  Numpy array or list of feature importance values
        of shape (n_features,).
    :param ax: Axes object to draw the plot onto, otherwise uses the current Axes.
    :param kwargs: Additional keyword arguments passed to matplotlib.axes.Axes.bar().
    :return: Axes object with the plot drawn onto it.
    :raises ValueError: If feature_names and feature_importance have different
        lengths or invalid input.
    """
    ...
    if len(feature_names) != len(feature_importance):
        raise ValueError(
            "feature_names and feature_importance must have the same length"
        )
