...
def plot_confusion_matrix(
    ...
    **kwargs,
    ...
    """Compute and plot confusion matrix and related classification scores.

    Calculates False Positive Rate, False Negative Rate, Accuracy, and F1 score.
    ...
    :param y_pred: array, shape = [n_samples]. Estimated targets as returned by a
        classifier.
    :param labels: array, shape = [n_classes]. List of labels to index the matrix.
    :param sample_weight: array-like of shape = [n_samples], optional.
        Sample weights for weighting the samples.
...
def _plot_confusion_matrix_helper(
    ...
    if len(labels) == 2:
        ...
        subplots[0].text(
            0,
            0.85,
            f"FPR: {np.array2string(1 - tnr, precision=2, separator=',')}",
        )
        subplots[0].text(
            0,
            0.35,
...
def plot_metric_growth_per_labeled_instances(
    ...
    **kwargs,
    ...
    Receives train and test sets, and plots the change in the given metric
    with increasing amounts of trained instances.

    :param X_train: array-like or sparse matrix of shape (n_samples, n_features).
        The training input samples.
    :param y_train: array-like of shape (n_samples,). The target values (class
    ...
    :param n_samples: List of numbers of samples for training batches, optional
        (default=None).
    :param quantiles: List of percentages of samples for training batches, optional
        (default=[0.05, 0.1, ..., 0.95, 1]). Used when n_samples=None.
    :param metric: sklearn.metrics API function which receives y_true and y_pred
        and returns float.
    :param random_state: int, RandomState instance or None, optional (default=None).
        Controls the shuffling applied to the data before applying the split.
    :param n_jobs: int or None, optional (default=None). The number of jobs to run
        in parallel.
    :param verbose: int, optional (default=0). Controls the verbosity when fitting
        and predicting.
    :param pre_dispatch: int or string, optional. Controls the number of jobs that
        get dispatched during parallel execution.
...
def _perform_data_partition_and_evaluation(
    ...
    # Ensure y_train is a NumPy array for consistent handling
    y_train_arr = np.asarray(y_train)

    if y_train_arr.ndim == 2 and y_train_arr.shape[1] == 1:
        classifier.fit(X_train, y_train_arr.ravel())
    else:
        # This handles 1D y_train_arr or multi-output y_train_arr
...
def visualize_accuracy_grouped_by_probability(
    ...
    **kwargs,
    ...
    Receives true test labels and classifier probability predictions,
    divides and classifies the results, and finally plots a stacked bar chart
    with the results. `Original code <https://github.com/EthicalML/XAI>`_.

    :param y_test: array, shape = [n_samples]. Ground truth (correct) target values.
    :param labeled_class: the class to inquire about.
    :param probabilities: array, shape = [n_samples]. Classifier probabilities for
        the labeled class.
    :param threshold: the probability threshold for classifying the labeled class.
    :param display_breakdown: if True, the results will be displayed as "correct"
        and "incorrect"; otherwise as "true-positives", "true-negatives",
...
def plot_roc_curve_with_thresholds_annotations(
    ...
    **kwargs,
    ...
    :param drop_intermediate: bool, default=True. Whether to drop some suboptimal
        thresholds which would not appear on a plotted ROC curve.
    :param average: {'micro', 'macro', 'samples', 'weighted'} or None,
        default='macro'. If not None, this determines the type of averaging
        performed on the data.
    :param max_fpr: float > 0 and <= 1, default=None. If not None, the
        standardized partial AUC over the range [0, max_fpr] is returned.
    :param multi_class: {'raise', 'ovr', 'ovo'}, default='raise'. Determines the
        type of configuration to use for multiclass targets.
    ...
    :param mode: str, default='lines+markers'. Determines the drawing mode for this
        scatter trace.
    :param add_random_classifier_line: bool, default=True. Whether to plot a
        diagonal dashed black line which represents a random classifier.
    ...
    for classifier_name, y_scores in classifiers_names_and_scores_dict.items():
        if y_true.shape != y_scores.shape:
            raise ValueError(
                f"Shape mismatch: y_true {y_true.shape} and y_scores "
                f"{y_scores.shape} for classifier {classifier_name}"
            )
...
def plot_precision_recall_curve_with_thresholds_annotations(
    ...
    **kwargs,
    ...
    :param positive_label: int, float, bool or str, default=None. The label of the
        positive class.
    :param sample_weight: array-like of shape (n_samples,), default=None. Sample
        weights.
    :param drop_intermediate: bool, default=True. Whether to drop some suboptimal
        thresholds which would not appear on a plotted Precision-Recall curve.
    :param fig: plotly's Figure object, optional. The figure to plot on.
    :param mode: str, default='lines+markers'. Determines the drawing mode for this
        scatter trace.
    :param add_random_classifier_line: bool, default=False. Whether to plot a
        diagonal dashed black line which represents a random classifier.
    :param show_legend: bool, default=True. Whether to display legend in the plot.
    :param kwargs: additional keyword arguments to be passed to the plot function.
    :return: The Figure object with the plot drawn onto it.
